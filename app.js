...
if(document.getElementById('regFoto')) {
    document.getElementById('regFoto').addEventListener('change', function(e) { 
        if(e.target.files[0]) {
            comprimirImagen(e.target.files[0], (base64) => { regBase64 = base64; }); 
        }
    });
}

if(document.getElementById('perfFile')) {
    document.getElementById('perfFile').addEventListener('change', function(e) {
        if(e.target.files[0]) {
            comprimirImagen(e.target.files[0], (base64) => {
                state.currentBase64 = base64;
                document.getElementById('perfDisplayFoto').src = base64; 
            });
        }
    });
}
...
function handleRegistro() {
    ...
    auth.createUserWithEmailAndPassword(email, pass).then((cred) => {
        ...
        db.ref(`Usuarios/${cred.user.uid}/perfil`).set({ 
            ...
            color: "#3b82f6" 
        }).then(() => alert("Bienvenido " + nombre + "!"));
    }).catch(e => alert(e.message));
}

function handleResetPassword() {
    auth.sendPasswordResetEmail(document.getElementById('resetEmail').value).then(() => { 
        alert("Enviado"); 
        toggleAuthForm('login'); 
    }).catch(e => alert(e.message));
}

function handleCambiarPassword(e) {
    e.preventDefault();
    ...
    if (user) {
        user.updatePassword(newPass).then(() => {
            ...
            if (error.code === 'auth/requires-recent-login') { 
                alert("Por seguridad, debes cerrar sesión y volver a entrar para poder cambiar tu contraseña."); 
            } else { 
                alert(error.message); 
            }
        });
...
function mostrarFraseDiaria() {
    const frases = [ 
        ...
        "Cuida de tus pequeños gastos; un pequeño agujero hunde un gran barco.", 
    ...
    if(document.getElementById('fraseMotivadora')) {
        document.getElementById('fraseMotivadora').innerText = `"${frases[diaDelAnio % frases.length]}"`;
    }
}

setTimeout(() => {
    const enterParaLoguear = (e) => { 
        if (e.key === 'Enter') { 
            e.preventDefault(); 
            handleLogin(); 
        } 
...
function closeDropdowns() { 
    ...
    if (fabMain && fabMain.classList.contains('active')) {
...
function cambiarTab(id, btn) {
    ...
    ['inCuenta', 'gaFuente', 'movOrigen', 'movDestino'].forEach(eid => { 
        if(document.getElementById(eid)) document.getElementById(eid).disabled = false; 
    });
...
function eliminarTransaccion(fid) { 
    if(!confirm("¿Borrar y devolver saldos?")) return; 
...
function renderAll() {
    ...
    state.cuentas.forEach(c => {
        ...
        if(c.diaPago && c.diaPago > 0) {
            ...
            if (yaPagado) { 
                aviso = `<br><small style="color:#a7f3d0; font-weight:bold;">✅ Pagado</small> <span onclick="db.ref('Usuarios/${auth.currentUser.uid}/cuentas/${c.id}/mesPagado').remove()" style="font-size:9px; cursor:pointer; text-decoration:underline;">(Deshacer)</span>`; 
            } else { 
                let textoDias = vence < 0 ? `⚠️ Atrasado` : (vence === 0 ? '🔥 ¡Paga HOY!' : `Faltan: ${vence}d`); 
                let colorTexto = vence <= 3 ? '#fca5a5' : '#e2e8f0'; 
                aviso = `<br><small style="color:${colorTexto}; font-weight:bold;">${textoDias}</small><br><button class="tb-action-btn" style="margin-top:4px; ${vence <= 0 ? 'background:#ef4444' : ''}" onclick="db.ref('Usuarios/${auth.currentUser.uid}/cuentas/${c.id}/mesPagado').set(${mesAct})">Marcar Pagado</button>`; 
            }
        }
        
        const colorFondo = getBankColor(c.banco);
        let limiteInfo = ""; 
        let tituloSaldo = c.tipo === 'credito' ? "DEUDA ACTUAL" : "SALDO DISPONIBLE";

        if (c.tipo === 'credito' && c.limite > 0) {
            const disponible = c.limite - c.saldo;
            limiteInfo = `
                <div style="text-align: right;">
                    <div style="font-size: 10px; opacity: 0.8;">Límite: $${c.limite.toLocaleString('es-MX')}</div>
                    <div style="font-size: 13px; font-weight: bold; color: #a7f3d0;">Disp: $${disponible.toLocaleString('es-MX', {minimumFractionDigits: 2})}</div>
                </div>`;
        }

        let digitosHtml = "";
        if (c.tipo !== 'efectivo') {
            digitosHtml = `<div class="tb-digitos">**** ${c.digitos || '0000'}</div>`;
        }

        const baseCardHtml = `
            <div class="tarjeta-bancaria" style="background: ${colorFondo};">
                <div class="tb-bg-shape tb-shape-1"></div>
                <div class="tb-bg-shape tb-shape-2"></div>
                <div class="tb-content">
                    <div class="tb-header">
                        <div style="display:flex; align-items:center; gap:10px;">
                            <img src="${c.icon}" style="width:28px; height:28px; border-radius:50%; background:white; padding:2px; object-fit:contain;">
                            <div>
                                <div class="tb-banco">${c.banco.toUpperCase()}</div>
                                ${digitosHtml}
                            </div>
                        </div>
                    ...
                    <div class="tb-footer">
                        <div>
                            <div class="tb-nombre">${c.nombre}</div>
                            ${aviso}
                        </div>
                        ${limiteInfo}
                    </div>
        ...
        const actionsHtml = `
                    <div class="tb-actions">
                        <button class="tb-action-btn" onclick="editCuenta('${c.id}')">✏️ Editar</button>
                        <button class="tb-action-btn" onclick="sumarInteres('${c.id}')">+ Interés</button>
                        <button class="tb-action-btn" onclick="const d=prompt('Dominio web del banco:'); if(d) db.ref('Usuarios/${auth.currentUser.uid}/cuentas/${c.id}/icon').set('https://www.google.com/s2/favicons?domain='+d+'&sz=128')">Logo</button>
                        <button class="tb-action-btn danger" onclick="if(confirm('¿Borrar cuenta permanentemente?')) db.ref('Usuarios/${auth.currentUser.uid}/cuentas/${c.id}').remove()">Borrar</button>
                    </div>
        `;

        if(c.tipo === 'debito' || c.tipo === 'efectivo'){ 
            tengo += c.saldo; 
            hDeb += baseCardHtml + `</div></div>`; 
        } else { 
            debo += c.saldo; 
            hCre += baseCardHtml + `</div></div>`; 
        }
    ...
    document.getElementById('listaIngresos').innerHTML = hI; 
    document.getElementById('listaMovimientos').innerHTML = hM;
...
// 9. CONFIGURACIÓN DE CUENTAS (CREAR, EDITAR, LOGOS)
...
function handleNuevaCuenta(e) { 
    ...
    const b = document.getElementById('cuBanco').value; 
    const tipo = document.getElementById('cuTipo').value;
